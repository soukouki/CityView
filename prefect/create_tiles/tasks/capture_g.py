...
@task(tags=["capture"], retries=5, retry_delay_seconds=20)
def capture_g(params: CreateTilesParams, tasks: list):
    ...
    for task in tasks:
        ...
        if check_exists(output_path):
            ...
            # 既存のスクリーンショット情報を取得
            screenshot_info = get_screenshot_by_tile(params.map_id, x, y)
...
def get_screenshot_by_tile(map_id: int, x: int, y: int):
