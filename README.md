...
- 移動タイル数 (DELTA)
  ...
  - 基本は推奨値そのままで良いと思います。
    ...
    - DELTAを小さくすると画像のサイズが小さくなるので、撮影時間が伸びる代わりにメモリ使用量が減ります。
...
- 糊代の割合 (%)
  - スクリーンショットを撮ると、指定した座標が画面の上下左右にズレます。このズレを吸収するのが糊代です。
...
- Prefectの動作が不安定です。Flow Runの様子を見守ってあげたり、場合によってはConcurrencyの値を調整したりしてください。
