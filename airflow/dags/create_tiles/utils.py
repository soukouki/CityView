...
def game_tile_to_screen_coord(tile_x: int, tile_y: int) -> tuple[int, int]:
    screen_x = PAKSET_SIZE * (tile_x - tile_y) + (ENABLE_WIDTH // 2) + IMAGE_MARGIN_WIDTH
    screen_y = (PAKSET_SIZE // 2) * (tile_x + tile_y) + (ENABLE_HEIGHT // 2) + IMAGE_MARGIN_HEIGHT
...
def game_tile_to_screen_lefttop_coord(tile_x: int, tile_y: int) -> tuple[int, int]:
    screen_x = PAKSET_SIZE * (tile_x - tile_y)
    screen_y = (PAKSET_SIZE // 2) * (tile_x + tile_y)
    return screen_x, screen_y
...
def screen_coord_to_game_tile(screen_x: int, screen_y: int) -> tuple[int, int]:
    ...
    tile_x = (X + 2 * Y) // (PAKSET_SIZE * 2)
    tile_y = (2 * Y - X) // (PAKSET_SIZE * 2)
    return tile_x, tile_y

# スクショ座標系と地図タイル座標系の変換式
def screen_coord_to_map_tile(screen_x: int, screen_y: int, z: int) -> tuple[int, int]:
    scale = 2 ** (MAX_Z - z)
    tile_x = (screen_x + (PAKSET_SIZE * 2) * MAP_TILES_Y + IMAGE_MARGIN_WIDTH * 2) // (TILE_SIZE * scale)
